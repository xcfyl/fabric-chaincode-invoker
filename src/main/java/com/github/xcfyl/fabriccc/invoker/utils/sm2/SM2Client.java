 ...
 */
public class SM2Client {

    public static final JcaPEMKeyConverter CONVERTER = new JcaPEMKeyConverter().setProvider(BouncyCastleProvider.PROVIDER_NAME);

    public SM2Client() throws NoSuchProviderException, NoSuchAlgorithmException, InvalidAlgorithmParameterException {
    ...
    public static String pemFrom(PrivateKey privateKey, String password) throws OperatorCreationException, IOException {
        ...
        try (JcaPEMWriter pemWriter = new JcaPEMWriter(sw)) {
            ...
            if (password != null && password.length() > 0) {
                encryptor = new JceOpenSSLPKCS8EncryptorBuilder(PKCS8Generator.AES_256_CBC).setProvider(BouncyCastleProvider.PROVIDER_NAME).setRandom(SecureRandomFactory.getSecureRandom()).setPasssword(password.toCharArray()).build();
            }
    ...
    public static X509Certificate loadX509CertificateFromFile(String filename) throws IOException, CertificateException, NoSuchProviderException {
    ...
    public static PublicKey derivePublicFromPrivate(PrivateKey privateKey) {
        ...
        return new BCECPublicKey(privateKey.getAlgorithm(), pubKeySpec, BouncyCastleProvider.CONFIGURATION);
    }
}